...
int readLBT_Threshold(char *file,vector<LBT_Threshold> &lbt_thr){
    ...
    while(fptr>>str){
        //cout<<endl<<str;
        index=str.find('=',0);
        lbtparam.parameter=str.substr(0,index);
        lbtparam.range=str.substr(index+1);
        index=lbtparam.range.find('-',0);
        lbtparam.LV=atoi((lbtparam.range.substr(0,index)).c_str());
        lbtparam.UV=atoi((lbtparam.range.substr(index+1)).c_str());
        lbt_thr.push_back(lbtparam);
        lc++;
    }
    fptr.close();
    return (EXIT_SUCCESS);
...
int readLBT(char *file,char *lbtextname,vector<LBTparam> &lbtparam){
    ...
    fits_movnam_hdu(fptr,BINARY_TBL,lbtextname,0,&status);
    if(status)  {fits_report_error(stderr,status); return (EXIT_FAILURE);  }
    long nrows=0;
    fits_get_num_rows(fptr,&nrows,&status);
    if(nrows==0){
        LOG(ERROR)<<"No data found in LBT";
        return (EXIT_FAILURE);
    }
    LBTparam param;
    for(long i=1;i<=nrows;i++){
        param.read(fptr,i);
        lbtparam.push_back(param);
    }
...
int LBTparam::read(fitsfile *fptr,int row){
     int status=0; 
