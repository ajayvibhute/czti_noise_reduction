...
int cztbunchclean::cztbunchcleanProcess()
{
    ...
    if (clobber == YES) {
        ...
        if (FileExists(outfile)||FileExists(livetimefile)) {
...
//    status=create_livetime_file(livetimefile);//commented by Mayuri,16th Dec 2017
	...
	for(i=0;i<nrows;i++)
	{
        ...
        //Read the RowNum column of bunch file
            if(nrows_bunch!=0) 
            {  
  ...
  */  
        ...
        for(i=0;i<nrows-1;i++)
        {
            ...
            if(nrows_bunch==0)
            {
                ...
                {
                    nbunch_evt++;
                    if(nbunch_evt==2) bunch_tstart=UTtime[i];
                }
                else
                {
