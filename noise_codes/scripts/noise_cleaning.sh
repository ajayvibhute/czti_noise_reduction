...
noise_cleaning_queue_file=$CZTIMONITOR/queues/noise_cleaning.queue
noise_cleaning_logfile=$CZTIMONITOR_LOG/noise_cleaning.log
...
do
	error_flag=0
	#Reading first line of the queue
	l2dir=$(head  -n 1 $noise_cleaning_queue_file)
	...
	if [ ! -z $l2dir ]; then
		...
		#temporary
		#czt bunch clean
		bunch_clean_evt=`echo $dsoutput| sed 's/.evt/_bc.evt/'`
		bunch_clean_livetime=`echo $m0l2evt| sed 's/.evt/_bc_livetime.fits/'`
		if [ $error_flag -eq 0 ];then	
		...
		echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S"):: cztbunchclean par_infile=$dsoutput par_outfile=$bunch_clean_evt par_bunchdeftime=30 par_bunch_length_thresh=20 par_skipT1=0 par_skipT2=0 par_skipT3=0 clobber=y history=y">>$noise_cleaning_logfile
			cztbunchclean par_infile=$dsoutput  par_bunchfile=$bunchfile par_outfile=$bunch_clean_evt par_livetimefile=$bunch_clean_livetime par_bunchdeftime=30 par_bunch_length_thresh=20 par_livetime_binsize=1 par_skipT1=0 par_skipT2=0 par_skipT3=0 clobber=y history=y>>$noise_cleaning_logfile
			p_status=$?
			if [ $p_status -ne 0 ];then
				error_flag=1
		...
		if [ $error_flag -eq 0 ];then
			...
			do
				...
				if [ $error_flag -eq 0 ];then
					...
					echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S"):: cztnoisypixclean $f $caldbbadpix $bunchfile $noiseconfigfile $f">>$noise_cleaning_logfile
				...
				if [ $error_flag -eq 0 ];then
					...
					echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S"):: cztsuperbunchclean $noisecleanedfile $bunchfile $noiselivetime $orbitno $sbcconfig $noisecleanedfile">>$noise_cleaning_logfile
				...
				if [ $error_flag -eq 0 ];then
					...
					echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S"):: cztheavybunchclean $superbunchcleanevt $bunchfile $caldb_path $caldblld $hbcconfig $superbunchcleanevt">>$noise_cleaning_logfile
				...
				if [ $error_flag -eq 0 ];then
					...
					echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S")::cztflickpixclean for $heavybunchcleanevt $noisebadpixfile $fcconfig $heavybunchcleanevt">>$noise_cleaning_logfile
				...
				if [ $error_flag -eq 0 ];then
					...
					echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S")::	czteventsep $flickcleanevt $f">>$noise_cleaning_logfile
				...
				if [ $error_flag -eq 0 ];then
					...
					echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S")::cztdatasel infile=$eventsepevt gtifile=$eventsepevt gtitype="quad" outfile=$dscleanevt clobber="y" history="y" ">>$noise_cleaning_logfile
		...
		echo "noise-cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S"):: Removing entry from $noise_cleaning_queue_file">>$noise_cleaning_logfile
		#Removing entry of the file from queue
		`sed -r -i "/^(.*$filename)(.*$orbno)$/d" $noise_cleaning_queue_file`
		echo "Script Execution completed"
		echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S"):: Script Execution completed">>$noise_cleaning_logfile
