		...
		if [ $error_flag -eq 0 ];then
			...
			do
				...
				if [ $error_flag -eq 0 ];then
					...
					echo "cztnoisypixclean infile=$f caldb_lld_file=$caldb_path$caldbbadpix thresholdfile=$noiseconfigfile outbadpixfile=$noisebadpixfile outfile=$noisecleanedfile clobber="y" history="y""

					echo "noise_cleaning.sh: $USER $(date "+%Y-%m-%d %H:%M:%S"):: cztnoisypixclean infile=$f caldb_lld_file=$caldb_path$caldbbadpix thresholdfile=$noiseconfigfile outbadpixfile=$noisebadpixfile outfile=$noisecleanedfile clobber="y" history="y"">>$noise_cleaning_logfile
